...
void AnimPainter::blocks_paint(cv::Mat image, int startMidiTime, int endMidiTime, int window_width, int window_height)
{
    ...
    cv::Point pt1_prev, pt2_prev, pt3_prev;
    int radius_prev = 0;
    pt5.x = 0;
    pt5.y = 0;
    ...
    for (std::list<MidiNote>::iterator it=notes.begin() ; it != notes.end(); ++it) // Run the list forwards
    {
        if ((*it).is_note == 1 && startMidiTime -50 < (*it).t_off && endMidiTime + 50 > (*it).t_on) // is_note checks if it's a real note to avoid getting trash.
        {
            ...
            y1 = (float)window_height/2 - (float)window_height*((float)(*it).pitch/((float)pitch_max - (float)pitch_min))*renderproperties->vertRange/50.0 - renderproperties->vertShift + window_height*renderproperties->vertRange/66;
            y2 = (float)window_height/2 - (float)window_height*(((float)(*it).pitch + 1.0)/((float)pitch_max - (float)pitch_min))*renderproperties->vertRange/50.0 - renderproperties->vertShift + window_height*renderproperties->vertRange/66;
            ...
            if (true) //(std::abs(pt1.x) < x_max && std::abs(pt1.y) < y_max && std::abs(pt2.x) < x_max && std::abs(pt2.y) < y_max);// && (*it).duration < max_duration) // This "if" is just to bypass an issue (certainly about big doubles being coverted to int), what causes random boxes appearing on the screen when one zooms in very close.
            {
                for (unsigned short tnum = 0; tnum < (tracks_count); tnum++)
                {
                    ...
                    if ((*it).track == tnum+1 && tracksproperties->active[tnum] == true && tracksproperties->interconnect[tnum] == 1) // All tracks - rectangle
                    {
                        ...
                        y3 = (float)window_height/2 - (float)window_height*(((float)(*it).pitch + 0.5)/((float)pitch_max - (float)pitch_min))*renderproperties->vertRange/50.0 - renderproperties->vertShift + window_height*renderproperties->vertRange/66;
                        ...
                        if (abs(pt3.x - pt5.x) < tracksproperties->max_connect_dist && pt5 != cv::Point(0,0))
                            if ( ! renderproperties->sep_render[1])
                                cv::line( image, pt3, pt5, {tracksproperties->getCv(tnum,2)*(*it).vel/512,  tracksproperties->getCv(tnum,1)*(*it).vel/512,  tracksproperties->getCv(tnum,0)*(*it).vel/512}, 1, 8 );
                            else
                                cv::line( img_buffer_sep_tracks[tnum], pt3, pt5, {tracksproperties->getCv(tnum,2)*(*it).vel/512,  tracksproperties->getCv(tnum,1)*(*it).vel/512,  tracksproperties->getCv(tnum,0)*(*it).vel*512}, 1, 8 );
                    ...
                    if ((*it).track == tnum+1 && tracksproperties->active[tnum] == true && tracksproperties->shape[tnum] == 4) // All tracks
                    {
                        ...
                        if (pt1_prev.x <= window_width/2 && pt1.x > window_width/2 && pt2_prev.x + 5 > window_width/2) // The note block is inside the center line // no more else if
                        {
                            float deriv = (((float)window_width/2 - (float)pt1_prev.x)/((float)pt1.x - (float)pt1_prev.x));
                            ...
                            if (pt1.x - pt2_prev.x > 5) // next_note t_on and old note t_off are far
                                if ( ! renderproperties->sep_render[0])
                                    cv::circle( image, pt3_prev, floor1(pt2_prev.x-window_width/2+1), {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
                                else
                                    cv::circle( img_playing_notes, pt3_prev, floor1(pt2_prev.x-window_width/2+1), {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
                            // Case 2: the end of the current note (prev) is close to the begin of the new note (legato) -> notes connect themselves
                            else
                                if ( ! renderproperties->sep_render[0])
                                    cv::circle( image, cv::Point(x_mov, y_mov), floor1(radius_prev + deriv*((float)radius - (float)radius_prev)), {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
                                else
                                    cv::circle( img_playing_notes, cv::Point(x_mov, y_mov), floor1(radius_prev + deriv*((float)radius - (float)radius_prev)), {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
...
//                        pt3.y = f2int_safe(y3); // Center y
                        ...
                        if (pt1_prev.x <= window_width/2 && pt1.x > window_width/2 && pt2_prev.x + 5 > window_width/2) // The note block is inside the center line // no more else if
                        {
                            float deriv = (((float)window_width/2 - (float)pt1_prev.x)/((float)pt1.x - (float)pt1_prev.x));
                            //int y_mov = (int) ((float)pt3_prev.y + deriv*((float)pt3.y - (float)pt3_prev.y));
                            int y1_mov = (int) ((float)pt1_prev.y + deriv*((float)pt1.y - (float)pt1_prev.y));
                            int y2_mov = (int) y1_mov + y2 - y1; // the difference among them remains the same
                            //int x_mov = window_width/2 + pt3_prev.x - pt1_prev.x + deriv*((pt3.x - pt1.x) - (pt3_prev.x - pt1_prev.x));
                            int x1_mov = window_width/2;
                            int x2_mov = window_width/2 + pt2_prev.x - pt1_prev.x - deriv*((pt2_prev.x - pt1_prev.x) - (pt2.x - pt1.x));
                            // Case 1: the distance to the next note is big (not a legato) -> note just fall off
                            if (pt1.x - pt2_prev.x > 5) // next_note t_on and old note t_off are far
                                if ( ! renderproperties->sep_render[0])
                                    cv::rectangle( image, cv::Point(- 2 + pt1_prev.x + deriv*(pt2_prev.x - pt1_prev.x), 2 + pt1_prev.y + deriv*(pt2_prev.y - pt1_prev.y)), cv::Point(2 + pt2_prev.x - deriv*(pt2_prev.x - pt1_prev.x), - 2 + pt2_prev.y - deriv*(pt2_prev.y - pt1_prev.y)), {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
                                else
                                    cv::rectangle( img_playing_notes, cv::Point(- 2 + pt1_prev.x + deriv*(pt2_prev.x - pt1_prev.x), 2 + pt1_prev.y + deriv*(pt2_prev.y - pt1_prev.y)), cv::Point(2 + pt2_prev.x - deriv*(pt2_prev.x - pt1_prev.x), - 2 + pt2_prev.y - deriv*(pt2_prev.y - pt1_prev.y)), {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
                            // Case 2: the end of the current note (prev) is close to the begin of the new note (legato) -> notes connect themselves
                            else
                                if ( ! renderproperties->sep_render[0])
                                    ...
                                    cv::rectangle( img_playing_notes, cv::Point(x1_mov, y1_mov), cv::Point(x2_mov, y2_mov), {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
...
//                        radius_prev = radius;
                    ...
                    if ((*it).track == tnum+1 && tracksproperties->active[tnum] == true && tracksproperties->shape[tnum] == 6) // All tracks - rectangle
                    {
                        x3 = (float)window_width*((-(float)startMidiTime + (float)(*it).t_middle)/((float)endMidiTime - (float)startMidiTime));
                        y3 = (float)window_height/2 - (float)window_height*(((float)(*it).pitch + 0.5)/((float)pitch_max - (float)pitch_min))*renderproperties->vertRange/50.0 - renderproperties->vertShift + window_height*renderproperties->vertRange/66;
                        pt1.y = f2int_safe(y3);
...
//                        }
                        if (pt1_prev.x <= window_width/2 && pt1.x > window_width/2 && pt2_prev.x + 5 > window_width/2) // The note block is inside the center line // no more else if
                        {
                            float deriv = (((float)window_width/2 - (float)pt1_prev.x)/((float)pt1.x - (float)pt1_prev.x));
                            //
                            int y1_mov = (int) ((float)pt3_prev.y + deriv*((float)pt3.y - (float)pt3_prev.y));
                            // int y2_mov = y1_mov;
                            ...
                            int x1_mov = window_width/2;
                            int x2_mov = window_width/2 + pt2_prev.x - pt1_prev.x - deriv*((pt2_prev.x - pt1_prev.x) - (pt2.x - pt1.x));
                            int x3_mov = window_width/2 + (pt2_prev.x - pt1_prev.x - deriv*((pt2_prev.x - pt1_prev.x) - (pt2.x - pt1.x)))/2;
                            ...
                            // Case 2: the end of the current note (prev) is close to the begin of the new note (legato) -> notes connect themselves
                                ...
                                if ( ! renderproperties->sep_render[0])
                                    cv::fillConvexPoly( image, ptos_mov, 4, {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
                                else
                                    cv::fillConvexPoly( img_playing_notes, ptos_mov, 4, {getColorTrackP(tnum, (*it).pitch).b*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).g*(*it).vel/64, getColorTrackP(tnum, (*it).pitch).r*(*it).vel/64}, -1, 8 );
