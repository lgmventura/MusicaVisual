...
void AnimPainter::paintNotes(MusicData mdt, cv::Mat image, std::vector <cv::Mat> img_buffer_sep_tracks, std::vector<cv::Mat> imgLayers, cv::Mat img_playing_notes, cv::Mat img_moving_notes, AnimWindow aw, std::list<LayerContainer> layers, RenderP rProp) // this function is called for every frame. aw.StartMidiTime is the time in the left side of the window, aw.EndMidiTime, at the right. These depend on playback position and zoom.
{
    ...
    // Loop through layers to draw everything that is behind the notes
    std::list<LayerContainer>::reverse_iterator lit = layers.rbegin();
    for(int iLayer = 0; lit != layers.rend(); lit++, iLayer++)
    {
        BlockLayers blockL = (*lit).Bl;
        imgLayers.push_back(cv::Mat::zeros(aw.Height, aw.Width, CV_8UC3));
        // =================== Vertical Lines =============
        ...
        // Chords: things plotted by layer, but not by note:
        // for chord layer:
        ...
        if ((*lit).Cl.CLType == ChordLayers::ChordLayerType::Tonnetz)
        {
            TonnetzRenderer::renderGrid(image, clCentre, RBuffer->TonnetzGridPositions, (*lit).Cl.CellDiameter, (*lit).Cl.HexLayout, (*lit).Cl.TonnetzShape, (*lit).Cl.GridColour);
        }
        else if ((*lit).Cl.CLType == ChordLayers::ChordLayerType::ChordStar)
        {
            Chord::circle type = (*lit).Cl.ChordStarType;
            if ( ! ((*lit).Cl.NoteNamesOnStar))
            {
                ChordStar::dispChordDisc(type, image, clCentre, diam, (*lit).Cl.GridColour, false, (*lit).Cl.TurnChordCircle, (*lit).Cl.AccidentalSharp);
            }
            else if ((*lit).Cl.NoteNamesOnStar)
            {
                ChordStar::dispChordDisc(type, image, clCentre, diam, (*lit).Cl.GridColour, true, (*lit).Cl.TurnChordCircle, (*lit).Cl.AccidentalSharp);
            ...
            for (cit = mdt.GChords.ChordsWTime.begin(), cit_next = ++mdt.GChords.ChordsWTime.begin(); cit_next!=(mdt.GChords.ChordsWTime.end()); ++cit, ++cit_next) // run through all chords
            {
                ...
                if (aw.CurrPosMiddle > chordWT_next.Start_time && (aw.CurrPosMiddle < chordWT.Start_time) && cit!=mdt.GChords.ChordsWTime.begin() && cit!=mdt.GChords.ChordsWTime.end())
                {
                    ...
                    this->paintChords((*cit).chord, chordProgress, image, aw, (*lit).Cl, rProp);
                    break;
                }
            }
        }
        imgLayers.push_back(image);
    ...
    for (std::list<MidiNote>::iterator it=mdt.Notes.begin() ; it != mdt.Notes.end(); ++it) // Run the list forwards
    {
        ...
        if ((*it).is_note == 1 && aw.StartMidiTime -50 < (*it).t_off && aw.EndMidiTime + 50 > (*it).t_on) // is_note checks if it's a real note to avoid getting trash.
        {
            ...
            for(int iLayer = 0; lit != layers.rend(); lit++, iLayer++)
            {
                ...
                double hZoom = hZoomGlob*(*lit).Bl.hZoomMult;
                ...
                aw.VZoom = (*lit).Bl.vZoomMult*rProp.vertRange/50.0;

                ChordLayers chordL = (*lit).Cl;
                BlockLayers blockL = (*lit).Bl;
                ...
                //std::cout << (*it).t_on << ' ' << (*it).t_off << ": " << (*it).pitch << '\n';
                //std::cout << spts.pt1.x << ' ' << spts.pt1.y << ": " << spts.pt2.x << ' ' << spts.pt2.y << '\n';
                if ((*lit).LType == LayerContainer::BlockLayer)
                {
                ...
                else  // chordLayer: things drawn for every note and for every layer
                {
                ...
                imgLayers.at(iLayer) = image;
            } // end loop through layers
        } // end check if note is visible in window
    ...
    lit = layers.rbegin();
    cv::Mat imgAux;
    image = cv::Mat::zeros(aw.Height, aw.Width, CV_8UC3);
    for(int iLayer = 0; lit != layers.rend(); lit++, iLayer++)
    {
        BlockLayers blockL = (*lit).Bl;
        imgAux = imgLayers.at(iLayer);
        ...
        cv::bitwise_and(image, imgLayers.at(iLayer), image);
    } // end loop through layers
}
